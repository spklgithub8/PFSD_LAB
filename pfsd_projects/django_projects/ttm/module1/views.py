...
def registerloginfunction(request):
    if request.method=='POST':
        ...
        if Register.objects.filter(email=email).exists():
            return HttpResponse("Email already registered. Use a different email.")
...
def weatherlogic(request):
    if request.method=='POST':
        ...
        API_KEY='d61533ada1c4c7cf7e162373f5d143e5'
...
def feedbackloginfunction(request):
    ...
    # Return a response after processing the request
    return HttpResponse("Thank you for your feedback!")

    # Return a response in case of GET request or other cases
